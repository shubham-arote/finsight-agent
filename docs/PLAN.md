...
| 5 | Eval harness (3 tiers + FinRAGBench-V subset adapter) | honest baseline report committed; CI fails on faithfulness/citation regression | ✅ 5a: full-agent eval (abstain/citation-hit/verified/claims + judge), CI-gated · 5b: FinRAGBench-V adapter + harness (seeded sample over source PDFs, corpus-wide retrieval, retrieval recall/MRR + page-level citation P/R + judge correctness + per-category), fixture-tested offline. Real run needs the HF dataset (queries_en.json + pdf_en.tar.gz) + keys; box-level citation eval via citation_labels = future |
